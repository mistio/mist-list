...
Polymer({
  _template: html`
    <style>
      ...
      paper-menu-button paper-button {
        ...
        padding-left: var(
          --mist-filter-paper-menu-button-paper-button-padding-left
        );
        text-transform: var(
          --mist-filter-paper-menu-button-paper-button-text-transform
        );
    ...
    <vaadin-dialog id="filterDialog" aria-label="styled">
      <template>
        ...
        <div class="buttons">
          ...
          <paper-button dialog-confirm="" on-tap="_saveFilter" class="confirm"
            >Save</paper-button
          >
        </div>
      </template>
  ...
  properties: {
    ...
    showSaveSearch: {
      type: Boolean,
      computed:
        '_computeShowSaveSearch(userFilter,combinedPresetFilters.length)',
    },
  ...
  _fetchSavedFilter: function () {
    ...
    current_filter_fallback_key =
      'mist-filter#' + this.id + '/all-resources/userFilter';
    ...
    try {
      ...
      if (JSON.parse(localStorage.getItem(saved_filters_key))) {
        this.set(
          'userSavedFilters',
          JSON.parse(localStorage.getItem(saved_filters_key))
        );
        this.dispatchEvent(
  ...
  addFilter: function (filter) {
    ...
    localStorage.setItem(
      saved_filters_key,
      JSON.stringify(this.userSavedFilters)
    );
    this.dispatchEvent(
      new CustomEvent('filter-change', {
        bubbles: true,
  ...
  _displayName: function (name, selectedPresetFilter, userFilter) {
    if (
      !userFilter &&
      (!this.selectedPresetFilter || !this.selectedPresetFilter.length)
    ) {
      ...
      if (filter) {
        ...
        return this.name
          ? '"' + userFilter + '"' + this.name.replace('All', '')
          : '"' + userFilter + '"';
      }
    }
  },
  ...
  _keyDownSearchInput: function (e) {
    ...
    if (e.keyCode == 27) {
      ...
      this.dispatchEvent(
        new CustomEvent('escape-pressed', {
          composed: true,
          bubbles: true,
        })
      );
